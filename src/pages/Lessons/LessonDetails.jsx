...
const LessonDetails = () => {
  ...
  useEffect(() => {
    ...
    const fetchLesson = async () => {
      try {
        ...
        const likedByMe =
          !!user?.email && Array.isArray(lessonData?.likes)
            ? lessonData.likes.includes(user.email)
            : false;
  ...
  // ========== Fetch Comments (✅ fixes reload vanish) ==========
  useEffect(() => {
    if (!id) return;
  ...
  const fetchSimilarLessons = async (category, tone) => {
    try {
      ...
      const similar = allLessons
        .filter(
          (l) =>
            l._id !== id && (l.category === category || l.emotionalTone === tone)
        )
        .slice(0, 6);
  ...
  useEffect(() => {
    if (!id) return;
    if (user?.email) checkIfFavorited();
  ...
  const handleToggleLike = async () => {
    ...
    try {
      ...
      // ✅ keep lesson count synced from backend
      setLesson((prev) =>
        prev
          ? {
              ...prev,
              likesCount:
                typeof newCount === "number"
                  ? newCount
                  : prev.likesCount + (likedNow ? 1 : -1),
            }
          : prev
      );

      toast.success(likedNow ? "Liked!" : "Like removed");
    } catch (error) {
          ...
          {comments.length === 0 ? (
            ...
            <div className="space-y-4">
              {comments.map((comment) => (
                <div key={comment?._id || `${comment?.createdAt}`} className="border-b border-gray-100 pb-3">
