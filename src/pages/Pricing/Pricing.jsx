...
const Pricing = () => {
  ...
  const { loadingUser, isPremium } = useUserInfo();
  ...
  const handleUpgrade = async () => {
    try {
      if (!axiosSecure) {
        console.error("axiosSecure not ready");
        return;
      }
  ...
  if (loadingUser) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-16 text-center">
        <p className="text-sm text-gray-600">Loading your plan information...</p>
      </div>
    );
  }
      ...
      <div className="grid md:grid-cols-2 gap-6 mb-10">
        ...
        <div className="border-2 border-orange-400 rounded-xl p-6 bg-[#FFF7ED] shadow-md">
          ...
          <p className="text-3xl font-bold text-gray-900 mb-4">
            ৳1500{" "}
            <span className="text-sm font-normal text-gray-600">/ lifetime</span>
          </p>
